...
export async function createStdioHarness(
  ...
  const client = new Client(
    { name: "mcp-mobile-toolkit-stdio-test-client", version: "0.1.0" },
    { capabilities: {} },
  );
  ...
  try {
    await withTimeout(
      ...
      startupTimeoutMs,
      "Timed out while starting the compiled MCP server.",
    );
  } catch (error) {
  ...
  assertProtocolStdout(): void {
    ...
    if (this.protocolErrors.length > 0) {
      throw new AggregateError(
        this.protocolErrors,
        "The compiled server wrote non-protocol bytes to stdout.",
      );
    }
  }
  ...
  private acceptProtocolLine(rawLine: string): void {
    ...
    try {
      ...
      this.protocolErrors.push(
        new Error(
          `stdout contained a non-protocol line (${line.length} bytes).`,
        ),
      );
    }
  }
