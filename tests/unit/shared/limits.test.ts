...
describe("shared limits", () => {
  ...
  it("enforces character, line, and collection boundaries inclusively", () => {
    expect(() =>
      assertCharacterLimit("123", 3, "descriptionCharacters"),
    ).not.toThrow();
    expect(() =>
      assertCharacterLimit("1234", 3, "descriptionCharacters"),
    ...
    expect(() =>
      assertCollectionLimit(500, 500, "feedbackItems"),
    ).not.toThrow();
    expect(() => assertCollectionLimit(501, 500, "feedbackItems")).toThrow(
      LimitExceededError,
    );
  });
  ...
  it("measures and enforces JSON depth", () => {
    ...
    expect(() => assertJsonDepthLimit({ a: { b: true } }, 1)).toThrow(
      LimitExceededError,
    );
  });
  ...
  it("rejects circular depth traversal and oversized aggregate JSON", () => {
    ...
    expect(() => assertAggregateJsonLimit({ value: "é" }, 5)).toThrow(
      LimitExceededError,
    );
  });
