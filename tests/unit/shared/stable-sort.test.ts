...
describe("stable total ordering", () => {
	...
	it("chains total comparators without locale-sensitive sorting", () => {
		...
		const comparator = chainComparators(
			compareBy((value: { severity: number }) => value.severity, reverseComparator(compareNumbers)),
			compareBy((value: { score: number }) => value.score, reverseComparator(compareNumbers)),
			compareBy((value: { id: string }) => value.id, compareStrings),
		);
