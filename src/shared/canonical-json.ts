...
export function canonicalJson(value: JsonValue): string {
	...
	const serialize = (current: JsonValue): string => {
		...
		if (Array.isArray(current)) {
			const array = current as readonly JsonValue[];
			for (let index = 0; index < array.length; index += 1) {
				if (!(index in array)) {
					throw new TypeError("Canonical JSON does not support sparse arrays.");
				}
			}
			result = `[${array.map((item) => serialize(item)).join(",")}]`;
		} else {
			const object = current as Readonly<Record<string, JsonValue | undefined>>;
			...
			const entries = Object.keys(object)
				...
				.map((key) => {
					...
					return `${serializeString(key)}:${serialize(objectValue)}`;
				});
			result = `{${entries.join(",")}}`;
		}
