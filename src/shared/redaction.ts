...
const JWT_PATTERN = /\beyJ[A-Za-z0-9_-]{2,}\.[A-Za-z0-9_-]{2,}\.[A-Za-z0-9_-]{2,}\b/gu;
const KNOWN_TOKEN_PATTERN = /\b(?:gh[opusr]_[A-Za-z0-9]{20,255}|github_pat_[A-Za-z0-9_]{20,255}|sk-[A-Za-z0-9_-]{20,255})\b/gu;
const EMAIL_PATTERN =
	/\b[A-Z0-9.!#$%&'*+/=?^_`{|}~-]{1,64}@[A-Z0-9](?:[A-Z0-9-]{0,61}[A-Z0-9])?(?:\.[A-Z0-9](?:[A-Z0-9-]{0,61}[A-Z0-9])?)+\b/giu;
const WINDOWS_PATH_PATTERN = /(?:[A-Za-z]:\\Users\\|\\\\[^\\\s]+\\[^\\\s]+\\Users\\)[^<>:"|?*\s]+/gu;
...
export function redactSensitiveText(value: string): string {
	return value
		.replace(
			...
			(_match, label: string, separator: string) => `${label}${separator}${REDACTION_MARKERS.secret}`,
		)
		...
		.replace(
			...
			(_match, label: string, separator: string) => `${label}${separator}${REDACTION_MARKERS.identifier}`,
		)
